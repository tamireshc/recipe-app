...
function FavoriteRecipes() {
  ...
  return (
    <div className="bg-stone-100 h-fit">
      ...
      {msgCopied && (
        <p className="text-center text-2xl text-red-700">Link copied!</p>
      )}
      ...
      <div className="flex flex-wrap justify-center bg-stone-100 px-4 mb-16">
        {favoriteRecipes &&
          favoriteRecipes.map((item, index) => (
            <div
              key={item.id}
              className="card bg-white rounded-lg p-4 ptext-center flex justify-center"
            >
              <div className="flex flex-wrap sm:flex-col justify-center items-center mx-auto">
                <div>
                  <Link
                    to={`/${
                      item.image.includes("meals") ? "foods" : "drinks"
                    }/${item.id}`}
                  >
                    <p
                      data-testid={`${index}-horizontal-name`}
                      className="font-alice text-2xl text-center text-stone-900 underline"
                    >
                      {item.name}
                    </p>
                  </Link>
              ...
              <div className="">
                <Link
                  to={`/${item.image.includes("meals") ? "foods" : "drinks"}/${
                    item.id
                  }`}
                >
