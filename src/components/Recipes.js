    ...
    if (filter !== "") {
      if (comida) {
        ...
        fetch(
          ...
          .then((raw) => raw.json())
          .then((data) => setFiltered(data.drinks));
      }
    }
  }, [filter]);
  ...
  return (
    <div>
      <section>
        <div className="bg-stone-200">
          ...
          <div className="flex flex-wrap justify-center pb-4">
            {categories
              .filter((item) => categories.indexOf(item) < cinco)
              .map((category) => (
                <button
      ...
      <section className="flex flex-wrap gap-4 bg-stone-100 justify-center py-8 pb-24">
        {!temFiltro
          ? recipes
              .filter((it) => recipes.indexOf(it) <= onze) // mostra os 12 primeiros
              .map(
                (
                  item,
                  i // cria os 'cards'
                  ...
                  >
                    ...
                    />
                    <p
                      className="font-mono text-center hover:underline mt-3"
          ...
          : filtered
              .filter((it) => filtered.indexOf(it) <= onze) // mostra os 12 primeiros, e condicionalmente renderiza a lista das categorias ou a lista da busca
              .map(
                (
                  item,
                  i // cria os 'cards'
